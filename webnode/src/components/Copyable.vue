<template>
    <div class="copyable" :class="{ copied }" @click="copy" tabindex="0">
        ...
        <slot></slot>
        <div class="tooltip" ref="tooltip">Copied</div>
    </div>
</template>
...
const CopyableProps = Vue.extend({
  props: {
    text: String
  }
})
...
export default class Copyable extends CopyableProps{
...
<style scoped>
    .copyable {
        ...
        padding: 0.5rem;
        cursor: pointer;
        transition: color .3s cubic-bezier(0.25,0,0,1);
    }
    .copyable:hover,
    .copyable:focus,
    .copyable.copied {
    ...
    .tooltip {
        ...
        opacity: 0;
        transition: transform .3s cubic-bezier(0.25,0,0,1), opacity .3s cubic-bezier(0.25,0,0,1);
        transition-delay: .2s;
    }
    .tooltip::after {
        content: '';
